...
function ContinentGamePlay() {
  ...
  const generateQuestion = () => {
    ...
    const correctCont = correct.continents[0];

    const wrongOptions = ALL_CONTINENTS
      .filter(c => c !== correctCont)
      .sort(() => Math.random() - 0.5)
      .slice(0, 3);

    setQuestion(correct);
    setOptions([...wrongOptions, correctCont].sort(() => Math.random() - 0.5));
    setTimeLeft(15);
  ...
  const handleAnswer = (answer) => {
    ...
    const isCorrect = answer === question?.continents[0];
  ...
  useEffect(() => {
    if (countries?.length > 0 && !question) generateQuestion();
  }, [countries, question]);
            ...
            <div className="lg:flex lg:gap-6">
              <div className="hidden lg:block lg:w-1/3">
                <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-5 md:p-6 hover:shadow-xl transition-shadow duration-300 sticky top-6">
                  <h1 className="text-lg font-bold text-slate-800 leading-none mb-2">Qitə Oyunu</h1>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className={`flex items-center justify-between px-4 py-3 rounded-xl transition-all duration-300 ${timeLeft < 5 ? "bg-red-50 text-red-600 animate-pulse shadow-md shadow-red-200" : "bg-slate-100 text-slate-700"}`}>
                      <div className="flex items-center gap-2"><Timer size={18} /><span className="font-medium">Vaxt</span></div>
                    ...
                    <div className="flex items-center justify-between px-4 py-3 bg-amber-50 text-amber-700 rounded-xl shadow-sm">
                      <div className="flex items-center gap-2"><Coins size={18} /><span className="font-medium">Xal</span></div>
              ...
              <div className="lg:w-2/3">
                <div className="lg:hidden bg-white rounded-2xl shadow-lg border border-slate-200 p-5 md:p-7 mb-4 sm:mb-6 hover:shadow-xl transition-shadow duration-300">
                  <div className="flex items-center justify-between gap-2">
                    <div>
                      <h1 className="text-xs sm:text-sm font-bold text-slate-800 leading-none mb-1">Qitə Oyunu</h1>
                      <p className="text-[10px] sm:text-xs text-slate-400 font-medium">{currentIdx + 1} / {TOTAL_QUESTIONS}</p>
                    </div>
                    <div className="flex items-center gap-2 sm:gap-4">
                      <div className="flex gap-0.5 sm:gap-1">
                        {[...Array(MAX_LIVES)].map((_, i) => (<Heart key={i} size={16} fill={i < lives ? "#ef4444" : "none"} className={`${i < lives ? "text-red-600 hover:scale-110 transition-transform" : "text-slate-200"}`} />))}
                      </div>
                      <div className={`flex items-center gap-1 px-2 py-1 rounded-xl transition-all duration-300 ${timeLeft < 5 ? "bg-red-50 text-red-600 animate-pulse shadow-md shadow-red-200" : "bg-slate-100 text-slate-700 hover:bg-slate-200"}`}>
                        <Timer size={14} /><span className="text-xs sm:text-sm font-bold tabular-nums">{timeLeft}s</span>
                      </div>
                      <div className="flex items-center gap-1 px-2 py-1 bg-amber-50 text-amber-700 rounded-xl hover:bg-amber-100 transition-colors duration-300 shadow-sm">
                        <Coins size={14} /><span className="text-xs sm:text-sm font-bold">{score}</span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex flex-col gap-4 sm:gap-6">
                  <div className="md:h-72 bg-white rounded-2xl shadow-lg border border-slate-200 p-3 sm:p-4 flex flex-col items-center hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                    ...
                    <p className="text-center text-slate-500 text-[11px] sm:text-sm font-medium mt-3 uppercase tracking-wider">
                      Bu hansı qitədə yerləşir?
                    </p>
                  </div>

                  <div className="grid grid-cols-2 gap-2 sm:gap-3">
                    {options.map((opt, i) => {
                      const isCorrect = opt === question?.continents[0];
                      ...
                      return (
                        <button key={i} disabled={!!selectedAnswer} onClick={() => handleAnswer(opt)} className={btnClass}>
                          <span className="font-bold text-[14px] sm:text-xl lg:text-2xl">{opt}</span>
                  ...
                  <div className="lg:hidden bg-white rounded-xl shadow-lg border border-slate-200 py-3 px-4 md:p-6 mb-4 sm:mb-6 hover:shadow-xl transition-shadow duration-300">
                    <div className="text-xs text-slate-500">
                      ...
                      <ul className=" md:leading-6 md:text-[16px] text-slate-400">
                        <li className="flex items-start gap-2"><span className=" font-bold">•</span><span>Hər düzgün cavab: +50 xal</span></li>
                        <li className="flex items-start gap-2"><span className=" font-bold">•</span><span>Hər səhv cavab: -1 can</span></li>
                        <li className="flex items-start gap-2"><span className=" font-bold">•</span><span>Vaxt bitdikdə səhv sayılır.</span></li>
                        <li className="flex items-start gap-2"><span className=" font-bold">•</span><span>Ümumi can sayı: 3</span></li>
                      </ul>
                    </div>
                  </div>
                </div>
            ...
            <div ref={gameResultRef} className="max-w-md mx-auto bg-gradient-to-br from-white to-slate-50 rounded-3xl shadow-2xl border border-slate-200 p-6 md:p-8 text-center hover:shadow-3xl transition-shadow duration-500">
              ...
              <div className="grid grid-cols-2 gap-3 mb-8">
                ...
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 shadow-md">
                  ...
                  <div className="text-xl font-black text-slate-800">{Math.floor(score / 50)} / {TOTAL_QUESTIONS}</div>
                </div>
              </div>
...
export default ContinentGamePlay