...
function ContinentGamePlay() {
  ...
  const generateQuestion = () => {
    ...
    const wrong = ALL_CONTINENTS.filter(c => c !== correctCont).sort(() => Math.random() - 0.5).slice(0, 3);
    setQuestion(correct);
    setOptions([...wrong, correctCont].sort(() => Math.random() - 0.5));
    setTimeLeft(15);
  ...
  const endGame = (finalScore) => {
    if (!savedRef.current) {
      ...
      saveGameResult({ gameType: "continent", score: finalScore, correct: Math.floor(finalScore / 50), total: TOTAL_QUESTIONS, timeMs: null });
    }
    setGameOver(true);
  };
  ...
  const handleUseHint = (hint) => {
    ...
    if (hint.id === "hint_5050") {
      const correct = question?.continents[0];
      const wrongOpts = options.filter(o => o !== correct);
      const toEliminate = wrongOpts.sort(() => Math.random() - 0.5).slice(0, 2);
  ...
  if (gameOver) return (
    <section className="min-h-screen py-8 px-4 bg-linear-to-br from-slate-50 to-blue-50 mt-14 md:mt-23 flex items-center justify-center">
      <GameResult score={score} totalQuestions={TOTAL_QUESTIONS} onRestart={restartGame} backTo="/games/continent" />
    </section>
  );
    ...
    <section className="min-h-screen py-4 px-3 sm:px-6 bg-linear-to-br from-slate-50 to-blue-50 mt-14 md:mt-23">
      <div className="max-w-6xl mx-auto">
        <div className="lg:flex lg:gap-6">
          <GameSidebar title="Qitə Oyunu" currentIdx={currentIdx} total={TOTAL_QUESTIONS} lives={lives} maxLives={MAX_LIVES} timeLeft={timeLeft} score={score} />
          <div className="lg:w-2/3">
            <GameHUD title="Qitə Oyunu" currentIdx={currentIdx} total={TOTAL_QUESTIONS} lives={lives} maxLives={MAX_LIVES} timeLeft={timeLeft} score={score} />
            ...
            <div className="flex flex-col gap-4 sm:gap-6 mt-4">
              <div className="md:h-72 bg-white rounded-2xl shadow-lg border border-slate-200 p-3 sm:p-4 flex flex-col items-center hover:shadow-xl transition-all">
                ...
                <p className="text-center text-slate-500 text-[11px] sm:text-sm font-medium mt-3 uppercase tracking-wider">{t("game.continentQ")}</p>
              </div>
              <div className="grid grid-cols-2 gap-2 sm:gap-3">
                {options.map((opt, i) => {
                  ...
                  if (selectedAnswer || isRevealedOpt) {
                    ...
                    cls += "bg-white border-slate-200 hover:border-blue-400 hover:bg-gradient-to-br hover:from-blue-50 hover:to-white hover:text-blue-700 text-slate-700 hover:shadow-lg hover:-translate-y-1";
                  }
                  return (
                    <button key={i} disabled={!!selectedAnswer || isRevealedOpt} onClick={() => handleAnswer(opt)} className={cls}>
