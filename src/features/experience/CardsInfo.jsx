...
function CardsInfo() {
    ...
    if (error) return (
        <div className="min-h-screen flex flex-col items-center justify-center gap-4">
            ...
            <button onClick={retry} className="px-6 py-2 bg-slate-900 text-white rounded-xl text-sm font-bold">Yenidən cəhd et</button>
        </div>
    );
    ...
    return (
        <section className="min-h-screen py-6 md:py-10 px-3 md:px-6 bg-linear-to-br from-slate-50 via-white to-slate-100 font-sans mt-13 md:mt-20">
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col items-center mb-6 md:mb-12 gap-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-6">
                    {filtered.map((country, index) => (
                        <div key={country.cca3 ?? index} className="h-36 md:h-52 cursor-pointer" style={{ perspective: "1000px" }} onClick={() => setFlippedCard(flippedCard === index ? null : index)}>
                            <div className="relative h-full w-full rounded-xl md:rounded-2xl transition-all duration-700" style={{ transformStyle: "preserve-3d", transform: flippedCard === index ? "rotateY(180deg)" : "rotateY(0deg)", }}>
                                <div className="absolute inset-0 h-full w-full rounded-xl md:rounded-2xl overflow-hidden border-[3px] md:border-[6px] border-white shadow-sm" style={{ backfaceVisibility: "hidden" }}    >
                                    <img src={country.flags.svg} alt={country.name.common} className="h-full w-full object-cover" />
                                </div>

                                <div className="absolute inset-0 h-full w-full rounded-xl md:rounded-2xl bg-slate-900 p-2 md:p-5 text-white border border-slate-800 flex flex-col justify-between overflow-hidden" style={{ backfaceVisibility: "hidden", transform: "rotateY(180deg)" }}    >
                                    ...
                                    <div className="space-y-1 md:space-y-3 my-auto">
                                        <div className="flex items-center gap-1.5 md:gap-2 bg-white/5 p-1 md:p-1.5 rounded-lg">
                                            <FaMapMarkerAlt className="text-red-500 text-[8px] md:text-xs shrink-0" />
                                            <span className="text-[9px] md:text-[12px] font-bold text-slate-200 truncate">{country.capital?.[0] ?? "---"}</span>
                                        </div>
                                        <div className="flex items-center gap-1.5 md:gap-2 bg-white/5 p-1 md:p-1.5 rounded-lg">
                                            <FaUsers className="text-emerald-400 text-[8px] md:text-xs shrink-0" />
                                            <span className="text-[9px] md:text-[12px] font-bold text-slate-200">
                                                {country.population > 1_000_000 ? (country.population / 1_000_000).toFixed(1) + "M" : (country.population / 1_000).toFixed(0) + "K"}
                                            </span>
                                        </div>
                                        <div className="flex items-center gap-1.5 md:gap-2 bg-white/5 p-1 md:p-1.5 rounded-lg">
                                            <FaCoins className="text-yellow-400 text-[8px] md:text-xs shrink-0" />
